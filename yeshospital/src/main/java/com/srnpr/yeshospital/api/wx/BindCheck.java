...
public class BindCheck extends RootApi<BindCheckResult, BindCheckInput> {
	...
	public BindCheckResult Process(BindCheckInput inputParam,
			MDataMap mRequestMap) {
		BindCheckResult result = new BindCheckResult();

		if (result.upFlagTrue()) {

			MDataMap map = DbUp.upTable("yh_member_extend_geracomium").one(
					"card_code", inputParam.getCardCode(), "member_name",
					inputParam.getUserName());
